...
session_start();
 ...
 settype($query,"integer");
	...
	if($result == "true")
	{
		...
		//$coll_user->update(array("cardno" => $query),array('$set' => array("points" => 0));
		//$coll_redeem->update(array('user.cardno'=>$query),array('$set' => array('user.$.points' => 0)));?>
		<script>alert('You have successfully purchased your product!');</script>
