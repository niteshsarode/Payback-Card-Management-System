...
session_start();
 ...
 $map=new MongoCode("function(){for(var idx=0;idx<this.user.length;idx++){var key=this.user[idx].cardno;
    for(var id=0;id<this.user[idx].shop_history.length;id++){
    var value=this.user[idx].shop_history[id].product_price;emit(key,value);}}}");
 $reduce=new MongoCode("function(keys,values){var sum=0;
    for(var id=0;id<values.length;id++){sum+=values[id];}return sum;}");
 $total=$db->command(array("mapreduce"=>"redeem","map"=>$map,"reduce"=>$reduce,"out"=>"mr"));
$z=$db->mr->find();
foreach($z as $obj)
{
    if($obj['_id']==$query)
...
<?php           
 ...
 foreach($cur_redeem as $obj)
          ...
          <?php 
            ...
            ?>
            <?php
           echo "<marquee><h3>We thank you for your trust.You have shopped worth RS. ".$y." with us.</h3></marquee>"; 
?>
